...
def config_dataset(config):
    ...
    if config["dataset"] == "covid":
        config["data_path"] = "./dataset/covid/"
    if config["dataset"] == "breakhis":
        config["data_path"] = "./dataset/breakhis/"
    config["data"] = {
...
def pr_curve_1(rF, qF, rL, qL, draw_range=set_range):
    ...
    for k in tqdm(draw_range):
        ...
        for it in range(n_query):
            ...
            p[it] = gnd_r / k
            r[it] = gnd_r / gnd_all
            print("k",k,"gnd",gnd.shape,"gnd_r",gnd_r,"gnd_all",gnd_all,"n_query",n_query)

            if k > 1000:
                if gnd_all == 1437 :
                    tn = np.append(tn, gnd_r)
                    fn = np.append(fn, (1437-gnd_r))

                if gnd_all == 644 :
                    tp = np.append(tp, gnd_r)
                    fn = np.append(fn, (644-gnd_r))
        
                
        P.append(np.mean(p))
        R.append(np.mean(r))
        TP.append(np.mean(tp))
        TN.append(np.mean(tn))
        FN.append(np.mean(fn))
    
    TP = int(TP[-1])
    TN = int(TN[-1])
    FN = int(FN[-1])
    FP = int(2081-(TP+FN+TN))
    return P, R,TP,TN,FN,FP
...
def confussion_matrix(rF, qF, rL, qL, datalength=datalength):
    #  https://blog.csdn.net/HackerTom/article/details/89425729
    n_query = qF.shape[0]
    Gnd = (np.dot(qL, rL.transpose()) > 0).astype(np.float32)
    Rank = np.argsort(CalcHammingDist(qF, rF))
    P, R = [], []
    for k in tqdm(datalength):
        p = np.zeros(n_query)
        r = np.zeros(n_query)
        for it in range(n_query):
            gnd = Gnd[it]
