...
def train_val(config, bit):
    ...
    for epoch in range(start_epoch, config["epoch"]+1):
        ...
        for image, label, ind in train_loader:
            ...
            mixed_loss =  0.7*loss + 0.3*loss_contrastive 
        ...
        if (epoch) % config["test_map"] == 0:
            ...
            print("train label",trn_label.shape,"tst_label",tst_label.shape)
            
            _, __,TP,TN,FN,FP = pr_curve(trn_binary.numpy(), tst_binary.numpy(
                ), trn_label.numpy(), tst_label.numpy())

            mAP, cum_prec, cum_recall,cum_confussion = CalcTopMap(trn_binary.numpy(), tst_binary.numpy(), trn_label.numpy(), tst_label.numpy(),
                                                   config["topK"])
            ...
            if mAP > Best_mAP:
                ...
                P, R,TP,TN,FN,FP = pr_curve(trn_binary.numpy(), tst_binary.numpy(
                ), trn_label.numpy(), tst_label.numpy())
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, features, labels,batch, mask=None):
        ...
        s = (torch.arange(batch_size * anchor_count).view(-1, 1).to(device))
